...
vblog "Starting a devc container in read mode by using 'rkt run' with stage1-fly. Running 'tail -f /dev/null' as a dummy process to keep the container going forever. Mounting $HOME/src to /src"
sudo rkt run --insecure-options=image --stage1-name=coreos.com/rkt/stage1-fly:1.20.0 /home/mprast/.devc_build/devc.aci --volume src,kind=host,source="$HOME/src",readOnly=false --mount volume=src,target=/src --exec "/usr/bin/tail" -- -f "/dev/null" &
